...
const plans = [
  { id: "basico", name: "Plan Básico", price: 8000, description: ["Acceso a 1 actividad.", "Carnet digital."] },
  { id: "completo", name: "Plan Completo", price: 12000, description: ["Acceso hasta 3 actividades.", "Carnet digital.", "10% de descuento."] },
  { id: "premium", name: "Plan Premium", price: 16000, description: ["Acceso a todas las actividades.", "Carnet digital.", "15% de descuento."] }
];
    ...
    <>
      <div className="formGroup">
        ...
        <input id="dni" className="formInput" autoComplete="on" type="number" placeholder="Ingrese su DNI"
          value={data.dni} onChange={(e) => handleChange("dni", e.target.value)} onKeyDown={preventArrows} />
      </div>
      <div className="formGroup">
      ...
      <div className="formGroup">
        ...
        <input id="apellido" className="formInput" placeholder="Ingrese su apellido" type="text"
          value={data.surname} onChange={(e) => handleChange("surname", e.target.value)} />
      </div>
      <div className="formGroup">
      ...
      <div className="formGroup">
        ...
        <input id="fechaNacimiento" autoComplete="off" className="formInput" type="date"
          value={data.birthdate} onChange={(e) => handleChange("birthdate", e.target.value)} />
      </div>
      <div className="formGroup">
        ...
        <input id="direccion" autoComplete="off" className="formInput" placeholder="Calle y altura del domicilio" type="text"
          value={data.address.street} onChange={(e) => handleAddress("street", e.target.value)} />
      </div>
      <div className="formGroup">
        ...
        <input id="localidad" className="formInput" type="text" placeholder="Ingrese localidad" onKeyDown={preventArrows}
          value={data.address.city} onChange={(e) => handleAddress("city", e.target.value)} />
      </div>
    </>
  );
};
  ...
  return (
    <>
      ...
      <div className="formGroup">
        ...
        <select id="securityQuestion" className="formInput" value={data.securityQuestion} onChange={(e) => handleChange("securityQuestion", e.target.value)}>
      ...
      <div className="formGroup">
        ...
        <input id="securityAnswer" className="formInput" placeholder="Respuesta a la pregunta de seguridad" type="text"
          value={data.securityAnswer} onChange={(e) => handleChange("securityAnswer", e.target.value)} />
      </div>
    </>
  );
};

const Step3 = ({ selectedPlan, onPlanSelect }) => {
  return (
    <>
      <div className="planContainer">
        ...
        <div className="planCardsContainer">
          {plans.map((plan) => (
            <div key={plan.id} className={`planCard ${selectedPlan === plan.id ? "selected" : ""}`}
...
function Signup() {
  ...
  const isStep2Valid = () => {
    return (formData.password && formData.password === passConfirm && formData.securityQuestion && formData.securityAnswer);
  };
  const isStep3Valid = () => {
    ...
    <>
      ...
      <div className="formContainer">
        ...
        <div className="formStep">
          <button className={`stepButton ${step === 1 ? "active" : ""}`}
            onClick={() => goToStep(1)}>
            1. Datos Personales
          </button>
          <button className={`stepButton ${step === 2 ? "active" : ""}`}
            disabled={!isStep1Valid()}
            onClick={() => goToStep(2)}>
            2. Seguridad
          </button>
          <button className={`stepButton ${step === 3 ? "active" : ""}`}
            disabled={!isStep2Valid() || !isStep1Valid()}
            onClick={() => goToStep(3)}>
            3. Plan
          </button>
          <button className={`stepButton ${step === 4 ? "active" : ""}`}
            disabled={!isStep3Valid() || !isStep2Valid() || !isStep1Valid() || isLoading}
            onClick={() => goToStep(4)}>
            4. Revisión
          </button>
        </div>
